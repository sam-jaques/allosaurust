use super::{
    ...
    Accumulator, Element, Error, MembershipWitness, NonMembershipWitness, ProofMessage, PublicKey,
};
...
impl MembershipProof {
    ...
    pub fn to_bytes(&self) -> [u8; Self::BYTES] {
        ...
        res[0..48].copy_from_slice(&self.e_c.to_affine().to_compressed());
        res[48..96].copy_from_slice(&self.t_sigma.to_affine().to_compressed());
        res[96..144].copy_from_slice(&self.t_rho.to_affine().to_compressed());
        res[144..176].copy_from_slice(&self.s_sigma.to_bytes());
        res[176..208].copy_from_slice(&self.s_rho.to_bytes());
        res[208..240].copy_from_slice(&self.s_delta_sigma.to_bytes());
        res[240..272].copy_from_slice(&self.s_delta_rho.to_bytes());
        res[272..304].copy_from_slice(&self.s_y.to_bytes());
        res
    }
    ...
    pub fn from_bytes(input: &[u8; Self::BYTES]) -> Result<Self, Error> {
        ...
        Ok(Self {
            ...
            t_rho: g1(&input[96..144])?,
            s_sigma: sc(&input[144..176])?,
            s_rho: sc(&input[176..208])?,
            s_delta_sigma: sc(&input[208..240])?,
            s_delta_rho: sc(&input[240..272])?,
            s_y: sc(&input[272..304])?,
        })
    }
}
...
impl fmt::Display for NonMembershipProof {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
mod tests {
    ...
    fn basic_membership_proof() {
        ...
        let msg = ProofMessage::Hidden { message: y.0 };
        let acc = Accumulator::with_elements(&sk, &[y]);
        let proof_params = ProofParams::new(pk, None);

        let mw = MembershipWitness::new(y, acc, &sk).unwrap();
    ...
    fn basic_nonmembership_proof() {
        ...
        transcript.append_message(b"accumulator", &acc.to_bytes());
        nmpc.get_bytes_for_challenge(&mut transcript);
        let challenge = Element::from_transcript(b"challenge", &mut transcript);
