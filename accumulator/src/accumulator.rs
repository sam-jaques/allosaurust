...
mod tests {
    ...
    fn one_year_updates() {
        ...
        let mut witness = MembershipWitness::new(items.len() - 1, &items, acc, &key).unwrap();
    ...
    fn false_witness() {
        ...
        let wit = MembershipWitness::new(Element::from(Scalar::from(101)), acc, &key);
        assert!(wit.is_none());
        let wit = MembershipWitness::new(Element::from(Scalar::one()), acc, &key);
        assert!(wit.is_some());
        let y = elems[1];
